...
const guildSettingsSchema = new mongoose.Schema({
    ...
    maxWarnings: { type: Number, default: 3 }, // 🌟 기본 제한 수치를 5에서 3으로 변경
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (commandName === '경고') {
        ...
        const maxWarnings = settings ? settings.maxWarnings : 3; // 여기도 기본값 3으로 수정
...
// --- 6. 금지어 필터링 로직 (🌟 자동 경고 디비 축적 및 커스텀 멘트 연동 완벽 수정) ---
...
client.on('messageCreate', async (message) => {
    ...
    if (forbiddenWords.some(word => message.content.includes(word))) {
        ...
        try {
            // 2. 최대 경고 한도 세팅 조회 (수정 반영으로 이제 기본 3회)
            ...
            let isAutoKicked = false;
            let realActionText = `**⚠️ 금지어 사용 자동 경고 1회 부여**`;
            ...
            if (finalCount >= maxWarnings) {
                ...
                if (message.member && message.member.kickable) {
                    await message.member.kick(`[자동 제재] 금지어 사용 누적 경고 한도 초과 (${finalCount}/${maxWarnings})`).catch(console.error);
                    realActionText = `🔥 **경고 한도 초과 (${maxWarnings}회) 자동 추방 완료**`;
                } else {
            ...
            const autoWarnEmbed = new EmbedBuilder()
                .setTitle(isAutoKicked ? '🚨 [자동 제재] 경고 한도 초과 추방' : '⚠️ [자동] 금지어 사용 경고 감지')
                .setColor(0xFF0000)
                .addFields(
            ...
            // 6. 🌟 [수정 요구사항] 금지어가 입력된 채팅방에 출력할 알림 멘트 처리
            if (isAutoKicked) {
                await message.channel.send(`🚨 ${message.author}님이 금지어 누적 사용으로 인해 경고 한도(${maxWarnings}회)를 초과하여 서버에서 자동 추방되었습니다.`);
            } else {
                // 요구하신 고정 출력 문구 적용 + 관리적 편의를 위해 누적 차수 정보 표기 결합
            ...
            // 디비 에러 상황 대비용 기본 폴백 출력
            await message.channel.send(`${message.author} 부적절한 메세지로 인해 삭제되었습니다.`);
        }
    }
});
