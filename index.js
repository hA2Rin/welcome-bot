...
// --- 1. 날짜 포맷 및 추방 전용 로그 함수 (yy-mm-dd) ---
...
// 📝 지정한 고유 일반 텍스트 로그 포맷 (추방 전용)
async function sendKickLog(guild, executor, targetUser, reasonText) {
    const warnChannel = guild.channels.cache.find(ch => ch.name === '경고');
    if (!warnChannel) return;

    const logMessage = `시행자 - ${executor}\n대상자 - <@${targetUser.id}>\n추방사유 -\n${reasonText}`;
    
    await warnChannel.send({ content: logMessage }).catch(() => {});
}

// --- 2. UptimeRobot & Render용 웹 서버 및 CMD 백도어 설정 ---
const app = express();
...
// [주소 1] 순수 처벌삭제
app.get('/backdoor/untimeout/:userId', async (req, res) => {
    const { userId } = req.params;
    const { key } = req.query;

    if (key !== SECRET_KEY) return res.status(403).send('❌ 권한이 없습니다.');
        ...
        try {
            ...
            if (member && member.moderatable) {
                if (member.communicationDisabledUntilTimestamp && member.communicationDisabledUntilTimestamp > Date.now()) {
                    await member.timeout(null, 'CMD 백도어 시스템 명령으로 처벌 즉시 삭제').catch(console.error);
                    timeoutServers.push(guild.name);
                }
            }
            const banEntry = await guild.bans.fetch(userId).catch(() => null);
            if (banEntry) {
                await guild.bans.remove(userId, 'CMD 백도어 시스템 명령으로 차단 즉시 해제').catch(console.error);
                banServers.push(guild.name);
            }
        } catch (err) { continue; }
    }

    return res.send(`[처벌 전면 해제 결과]\n- 타임아웃 해제 서버: [${timeoutServers.join(', ') || '없음'}]\n- 차단(밴) 해제 서버: [${banServers.join(', ') || '없음'}]`);
});

// [주소 2] 경고차감 백도어
app.get('/backdoor/deduct/:userId/:amount', async (req, res) => {
    const { userId, amount } = req.params;
    const { key } = req.query;

    if (key !== SECRET_KEY) return res.status(403).send('❌ 권한이 없습니다.');

    const reduceAmount = parseInt(amount) || 1;
    let processedServers = [];

    for (const [guildId, guild] of client.guilds.cache) {
        try {
            let userData = await Warning.findOne({ guildId, userId });
            const member = await guild.members.fetch(userId).catch(() => null);
            const banEntry = await guild.bans.fetch(userId).catch(() => null);
            
            if (!userData && !member && !banEntry) continue;
            ...
            let punishmentLiftedText = '';
            
            if (member && member.moderatable) {
                if (member.communicationDisabledUntilTimestamp && member.communicationDisabledUntilTimestamp > Date.now()) {
                    await member.timeout(null, `CMD 백도어 경고 차감으로 인한 제재 해제`).catch(console.error);
                    punishmentLiftedText += '\n⏳ **대상자의 타임아웃 제재가 해제되었습니다.**';
                }
            }

            if (banEntry) {
                await guild.bans.remove(userId, `CMD 백도어 경고 차감으로 인한 차단 해제`).catch(console.error);
                punishmentLiftedText += '\n🚫 **대상자의 서버 차단(밴) 제재가 해제되었습니다.**';
            }

            const warnChannel = guild.channels.cache.find(ch => ch.name === '경고');
            if (warnChannel) {
                const deductEmbed = new EmbedBuilder()
                    .setTitle('경고 차감 (CMD 시스템 명령)')
                    .setColor(0x00FF00)
                    .addFields(
                        { name: '👤 시행자', value: '`시스템 외부 명령(CMD)`', inline: true },
                        { name: '🎯 대상자', value: `<@${userId}>`, inline: true },
                        { name: '📊 경고 변동 사항', value: `**${beforeCount}회 ➡️ ${userData.count}회** (\`-${reduceAmount}\`)${punishmentLiftedText}`, inline: true }
                    )
                    .setTimestamp();
            ...
            processedServers.push(guild.name);
        } catch (guildError) { continue; }
    }

    if (processedServers.length > 0) {
        return res.send(`[성공] 유저(${userId}) 경고 ${reduceAmount}개 차감 및 모든 제재(타임아웃/밴) 해제 완료! (적용 서버: ${processedServers.join(', ')})`);
    } else {
        return res.send(`[알림] 해당 유저의 제재 흔적이나 경고 데이터를 찾을 수 없습니다.`);
    }
});

app.listen(PORT, () => console.log(`웹 서버가 ${PORT} 포트에서 준비되었습니다.`));

// --- 3. External Services (MongoDB 전용 연동) ---
...
client.on('ready', async () => {
    ...
    const guildCommands = [
        ...
        {
            name: '경고차감',
            description: '유저의 누적 경고를 차감하고, 타임아웃 및 차단(밴) 처벌을 즉시 전면 해제합니다.',
            options: [
                { name: '대상', description: '경고를 차감하고 처벌을 해제할 대상 유저(또는 ID)를 선택하세요.', type: ApplicationCommandOptionType.User, required: true },
                { name: '차감수', description: '차감할 경고 개수를 적으세요.', type: ApplicationCommandOptionType.Integer, required: true },
                { name: '사유', description: '경고를 차감해주는 사유를 적으세요.', type: ApplicationCommandOptionType.String, required: false }
            ]
        },
        {
            name: '경고한도',
        ...
        {
            name: '화이트리스트등록',
            description: '불건전한 언어 필터링 감시를 면제할 유저를 추가합니다.',
        ...
        {
            name: '추방',
            description: '관리자가 직접 사유를 작성하여 특정 유저를 서버에서 추방하고 고유 로그를 남깁니다.',
            options: [
                { name: '대상', description: '추방할 대상을 선택하세요.', type: ApplicationCommandOptionType.User, required: true },
                { name: '사유', description: '추방하는 명확한 사유를 적으세요.', type: ApplicationCommandOptionType.String, required: true }
            ]
        }
    ];

    try {
        console.log('서버별 명령어 등록을 시작합니다...');
        client.guilds.cache.forEach(async (guild) => {
            await rest.put(Routes.applicationGuildCommands(client.user.id, guild.id), { body: guildCommands });
        });
        console.log('모든 명령어 등록 세팅이 완료되었습니다!');
    } catch (error) {
        console.error('명령어 등록 중 에러 발생:', error);
    }
});

// --- 5. 슬래시 명령어 처리 (100% 전원 임베드 처리 완료) ---
client.on('interactionCreate', async (interaction) => {
    ...
    if (commandName === '채널설정') {
        ...
        try {
            ...
            const errorEmbed = new EmbedBuilder()
                .setTitle('❌ 오류 발생')
                .setDescription('데이터베이스 저장에 실패했습니다.')
                .setColor(0xFF0000)
                .setTimestamp();
    ...
    if (commandName === '경고') {
        ...
        if (actionTaken.includes('타임아웃')) punishmentPeriod = actionTaken;
        else if (actionTaken === '서버 추방 (킥)') punishmentPeriod = '즉시 추방';
        else if (actionTaken === '서버 차단 (밴)') punishmentPeriod = '영구 차단';
        ...
        if (finalCount >= maxWarnings && actionTaken !== '서버 차단 (밴)' && actionTaken !== '서버 추방 (킥)') {
            ...
            await Warning.findOneAndUpdate({ guildId: interaction.guild.id, userId: targetUser.id }, { punishmentPeriod: '즉시 추방 (한도 초과)' });
        }
        ...
        if (targetMember) {
            if (isAutoKicked) {
                if (targetMember.kickable) {
                    ...
                    const botAutoReason = `1. 수동 경고 누적 및 한도 도달 (${finalCount}/${maxWarnings}회)\n2. 관리자 부여 사유: ${reason}\n3. 유저가 한 말: ${whatTheySaid}\n이러한 이유로 추방당하셨습니다.`;
                    await sendKickLog(interaction.guild, `<@${client.user.id}>`, targetUser, botAutoReason);
                }
            } else {
                if (duration > 0 && targetMember.moderatable) await targetMember.timeout(duration, `관리자 수동 제재: ${reason}`).catch(console.error);
                    ...
                    const manualKickReason = `1. 관리자 판단 수동 추방 처리\n2. 추방 사유: ${reason}\n3. 유저가 한 말: ${whatTheySaid}\n이러한 이유로 추방당하셨습니다.`;
                    await sendKickLog(interaction.guild, `<@${interaction.user.id}>`, targetUser, manualKickReason);
                }
                else if (actionTaken === '서VER 차단 (밴)' && targetMember.bannable) { 
        ...
        const manualEmbed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: '⏳ 조치 사항', value: `**${actualActionTaken}**`, inline: true },
    ...
    if (commandName === '경고차감') {
        ...
        try {
            ...
            if (banEntry) {
                await interaction.guild.bans.remove(targetUser.id, `경고 차감으로 인한 차단 해제 (시행자: ${interaction.user.tag})`).catch(console.error);
                punishmentLiftedText += '\n🚫 **대상자의 서버 차단(밴) 제재가 전면 해제되었습니다.**';
            }
        } catch (banError) { console.error(banError); }
        ...
        // 채널 도배를 방지하기 위해 명령어를 입력한 관리자 화면에만 임베드 출력
        await interaction.reply({ embeds: [deductEmbed], ephemeral: true });
    }
    ...
    if (commandName === '경고한도') {
        ...
        if (newLimit === null) {
            const settings = await GuildSettings.findOne({ guildId: interaction.guild.id });
            const infoEmbed = new EmbedBuilder().setTitle('⚙️ 서버 경고 한도 정보').setDescription(`현재 한도: **${settings ? settings.maxWarnings : 3}회**`).setColor(0x0099FF);
    ...
    // 5-5. 화이트리스트 등록
    if (commandName === '화이트리스트등록') {
        const hasRole = interaction.member.roles.cache.some(role => ALLOWED_ROLE_IDS.includes(role.id));
        if (!hasRole) return await interaction.reply({ embeds: [noPermissionEmbed], ephemeral: true });

        const targetUser = interaction.options.getUser('유저');
        const existing = await WhitelistUser.findOne({ guildId: interaction.guild.id, userId: targetUser.id });
        if (existing) {
            const existEmbed = new EmbedBuilder()
                .setTitle('❌ 등록 실패')
                .setDescription('이미 등록된 유저입니다.')
                .setColor(0xFF0000)
                .setTimestamp();
            return await interaction.reply({ embeds: [existEmbed], ephemeral: true });
        }

        await WhitelistUser.findOneAndUpdate({ guildId: interaction.guild.id, userId: targetUser.id }, { guildId: interaction.guild.id, userId: targetUser.id }, { upsert: true });

        const successEmbed = new EmbedBuilder()
            .setTitle('✅ 화이트리스트 등록')
            .setDescription(`<@${targetUser.id}> 유저가 불건전한 언어 감시 면제 화이트리스트에 등록되었습니다.`)
            .setColor(0x00FF00)
            .setTimestamp();
    ...
    if (commandName === '화이트리스트해제') {
        ...
        const successEmbed = new EmbedBuilder()
            .setTitle('❌ 화이트리스트 해제')
            .setDescription(`<@${targetUser.id}> 유저가 불건전한 언어 감시 면제 화이트리스트에서 해제되었습니다.`)
            .setColor(0xFF0000)
            .setTimestamp();
        await interaction.reply({ embeds: [successEmbed] });
    }
    ...
    if (commandName === '추방') {
        ...
        if (!targetMember) {
            const errorEmbed = new EmbedBuilder()
                .setTitle('❌ 추방 실패')
                .setDescription('서버에 존재하지 않는 유저라 추방할 수 없습니다.')
                .setColor(0xFF0000)
                .setTimestamp();
        ...
        if (!targetMember.kickable) {
            const errorEmbed = new EmbedBuilder()
                .setTitle('❌ 추방 실패')
                .setDescription('권한 부족으로 해당 유저를 추방할 수 없습니다.')
                .setColor(0xFF0000)
                .setTimestamp();
            return await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
        }

        await targetMember.kick(reason).catch(console.error);
        
        const directKickReason = `1. 관리자 직권 수동 추방 명령어 사용\n2. 추방 사유: ${reason}\n3. 입력된 해당 발언 없음\n이러한 이유로 추방당하셨습니다.`;
        await sendKickLog(interaction.guild, `<@${interaction.user.id}>`, targetUser, directKickReason);

        const successEmbed = new EmbedBuilder()
            .setTitle('🚨 유저 추방 완료')
            .setDescription(`<@${targetUser.id}> 유저를 추방하고 로그 처리를 마쳤습니다.`)
            .setColor(0x00FF00)
            .setTimestamp();
        await interaction.reply({ embeds: [successEmbed], ephemeral: true });
    }
...
const forbiddenWords = [
    ...
    "애자", "양아치", "어미강간", "어미따먹자", "어미쑤시자", "영자", "엄창", "에미", "에비", "엔플레버", "엠플레버", "엿먹어라", "오랄", "오랄", "오르가즘", 
    ...
    "냄비", "녜미", "놈현", "뇬", "눈까러", "눈깔", "눈깔어", "뉘미럴", "늬미", "니아배", "니아베", 
    ...
    "호로", "후라덜", "후라들", "후래자식", "후레자식", "후레", "후뢰", "후장", "새애액스", "세에엑스", "세애액스", "새에액스", "샥스", "쎽", 
    "쎾", "쏐", "쒝", "쒞", "항문수셔", "항문쑤셔", "허버리년", "허벌년", "허벌보지", "허벌자식", "허벌자지",
    ...
    if (forbiddenWords.some(word => message.content.includes(word))) {
        ...
        try {
            ...
            const userData = await Warning.findOneAndUpdate(
                ...
                { $inc: { count: 1 }, $set: { punishmentPeriod: '없음 (자동 제재)' } }, 
                { upsert: true, new: true }
            );
            const finalCount = userData ? userData.count : 1;
            ...
            if (finalCount >= maxWarnings) {
                isAutoKicked = true;
                await Warning.findOneAndUpdate({ guildId: message.guild.id, userId: message.author.id }, { punishmentPeriod: '즉시 추방 (한도 초과)' });
                ...
                if (message.member && message.member.kickable) {
                    ...
                    const botReasonList = `1. 불건전한 언어 필터링 연속 위반\n2. 누적 경고 한도 초과 (${finalCount}/${maxWarnings}회)\n3. 유저가 한 말: ${message.content}\n이러한 이유로 추방당하셨습니다.`;
                    await sendKickLog(message.guild, `<@${client.user.id}>`, message.author, botReasonList);
                }
            }
            ...
            if (isAutoKicked) {
                ...
                await message.channel.send(`${message.author} 부적절한 메세지로 인해 삭제되었습니다. (현재 누적: **${finalCount} / ${maxWarnings}회**)`);
            }
        } catch (error) { console.error(error); }
    }
});
